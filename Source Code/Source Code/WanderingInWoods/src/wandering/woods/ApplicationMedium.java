...
public class ApplicationMedium extends JFrame implements KeyListener {
	...
	private void resetPosition() {
		...
		for (int i = 1; i <= Integer.parseInt(noOfPlayers.getText()); i++) {
			position[Integer.parseInt(playerRow[i - 1].getText())][Integer.parseInt(playercol[i - 1].getText())] = ""
					+ i;
	...
	private void movePosition(int i, int j, int newRow, int newCol) {
		...
		if (players.stream()
				.filter(p -> position[newRow][newCol].contains("-") || p.getSymbol().equals(position[newRow][newCol]))
				.findAny().isPresent()) {
			try {
	...
	private static boolean gameInputs() {
		...
		if (result == JOptionPane.OK_OPTION) {
			try {
				...
				if (Integer.parseInt(noOfPlayers.getText()) < 2 || Integer.parseInt(noOfPlayers.getText()) > 4) {
					errorMessage.setText("Number of players must be above 2 and less than 4");
					return false;
				}
				for (int i = 1; i <= Integer.parseInt(noOfPlayers.getText()); i++) {
